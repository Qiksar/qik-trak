...
import QikTrakNames from './qiktrak-names';
...
export default class QikTrakHasura {
  ...
  async UntrackTable(table_name: string) {
    var query = {
  ...
  async createRelationships(relationship: any) {
    ...
    this.config.Logger.Log(
      '    ARRAY RELATIONSHIP - ' +
        array_rel_spec.args.name +
        ' -> ' +
        relationship.referencing_table +
        ' where ' +
        relationship.referencing_table +
        '.' +
        relationship.referencing_key +
        ' matches ' +
        relationship.referenced_table +
        '.' +
        relationship.referenced_key,
    );
  ...
  async createRelationship(relSpec: any) {
  ...
  async executeSqlScript(scriptFilename: string) {
  ...
  async runSQL_Query(sql_statement: string) {
    ...
    return await this.runGraphQL_Query('/v2/query', sqlQuery)
      ...
      .catch((e) => {
        ...
        this.config.Logger.Log('--------------------------------------------------------------');
        this.config.Logger.Log('');
        this.config.Logger.Log('QIKTRAK: ERROR');
        this.config.Logger.Log('');
        ...
        if (!e.response) this.config.Logger.Log('Error Message : ' + e);
        else this.config.Logger.Log('Error Message : ' + JSON.stringify(e.response.data));
  ...
  async runGraphQL_Query(endpoint: string, query: any) {
    if (!endpoint) throw 'endpoint is required';
    if (!query) throw 'query is required';
    if (!this.config.hasuraAdminSecret) throw 'hasuraAdminSecret is required';
  ...
  //
  async generateJsonView(viewFile: string) {
    const content = fs.readFileSync(viewFile, { encoding: 'utf8' });
    const root = JSON.parse(content);

    const promises = root.views.map(async (view: any) => {
...
`;
    ...
    if (view.columns) {
      ...
      view.columns.jsonValues.map((col: any) => {
        view_columns += `CAST(${view.columns.jsonColumn} ->> '${col.jsonName}' AS ${col.sqlType}) AS "${col.sqlName}",`;
      });
    }
...
`;
    ...
    if (view.relationships) {
      view.relationships.map(async (relationship: any) => {
